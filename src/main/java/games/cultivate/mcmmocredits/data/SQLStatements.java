...
enum SQLStatements {
    ...
    DOES_PLAYER_EXIST("SELECT * FROM `MCMMOCredits` WHERE `UUID`= ? LIMIT 1;"),
    SQLITE_CREATE_TABLE("CREATE TABLE IF NOT EXISTS MCMMOCredits(id INTEGER PRIMARY KEY AUTOINCREMENT, UUID VARCHAR NOT NULL, last_known_name VARCHAR NOT NULL, credits INT);"),
    MYSQL_CREATE_TABLE("CREATE TABLE IF NOT EXISTS `MCMMOCredits`(`id` int PRIMARY KEY AUTO_INCREMENT,`UUID` text NOT NULL,`last_known_name` text NOT NULL,`credits` int);");
